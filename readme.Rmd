...
Note that the notebooks and their snippets must be executed in order, and that not all notebooks should be executed in all contexts, i.e. when running in the VM, setup (step 1) and Getting github project URLs (step 2) were aready executed and when accessing the entire dataset on server, only reporting (steps 8 and 9) can be ran. 
